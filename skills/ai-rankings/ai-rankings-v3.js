...
// 保存报告到文件
function saveReportToFile(content) {
  const date = new Date();
...
function generateEnhancedReport() {
  ...
  analysisSection += `- **需要长上下文**: ${AA_DATA.context[0].model} (10M tokens)\n`;
...
if (require.main === module) {
  ...
  console.log(report);
  console.log('\n__JSON_OUTPUT__');
  console.log(JSON.stringify({ content: report, savedPath }));
}

module.exports = { generateEnhancedReport, analyzeTrends, saveReportToFile };
